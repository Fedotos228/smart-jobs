...
export default function Languages({ dark = false }: { dark?: boolean }) {
  const [active, setActive] = useState<ILanguage>(languageMap[0])
  ...
  return (
    <div className={cn(
      ...
      <div
        className='inline-flex items-center gap-3.5 pr-4'
      ...
      >
        ...
        <div className='flex items-center gap-1.5 py-3 px-4'>
          ...
          <span className={cn('transition-colors duration-300', dark ? 'text-foreground' : 'text-white')}>{active.name}</span>
        </div>
